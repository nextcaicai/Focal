...
> 关联文档：`docs/embedding-search-governance.md`（产品决策：不全库 embed、关键词全库搜）  
...
| 无语义关键词命中（意译查询）       | 关键词全库；语义不补捞（阶段 2） |
...
| 2026-07-12 | 无语义关键词时不跑全库语义             | 乱串零结果；关键词全库为默认                           |
| 2026-07-12 | 产品决策见 embedding-search-governance | 不全库 embed；入库即已读不 embed；关键词全库为默认搜索 |

---

## 9. 关联：Embedding 产品决策

启动/搜索性能优化（本文 P0/P1）解决「怎么加载、怎么搜得快」。

产品边界与 indexing 范围见 **[Embedding 与搜索治理方案](./embedding-search-governance.md)**：

- 全库 embedding：**不要**（默认）
- 关键词全库搜索：**要**（默认路径）
- 入库即已读 embed：**不要**

实施顺序：本文档 P0/P1（阶段 0 止血）→ embedding-search-governance 阶段 1 减负 → 阶段 2 搜得准。
